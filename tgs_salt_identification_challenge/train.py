...
def train(model, epochs, batch_size, trainX, trainY, valX, valY):
    ...
    model_checkpoint = tf.keras.callbacks.ModelCheckpoint("./data/models/keras.model", monitor='val_my_iou_metric', mode='max', save_best_only=True, verbose=1)
    ...
    try:
        ...
        ioudf.plot.line()
        plt.savefig('./data/outputs/figures/train_iou.png')
        # plt.show()
        lossdf.plot.line()
        plt.savefig('./data/outputs/figures/train_loss.png')
        # plt.show()
        dlossdf.plot.line()
        plt.savefig('./data/outputs/figures/train_dice_loss.png')
        # plt.show()
        accdf.plot.line()
        plt.savefig('./data/outputs/figures/train_accuracy.png')
        # plt.show()
    except Exception as e:
        log.error('save training images error')
